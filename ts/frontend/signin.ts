...
/**
 * @fileoverview Front-end methods for signing in.
 *
...
async function updateFirebase(
  ...
  let result = await fu.updateCollection(
    db, collA, collB, doc,
    (_x: string, y: string[]) => {
...
async function update(authResult: any) {
  ...
  await updateFirebase(
    lu.getFirebase().firestore(), fc.TestsCollection,
    authResult.user.uid, fc.NemethCollection);
...
export function init(fireui: any, id: string, config = uiConfig) {
  let ui = new fireui.auth.AuthUI(lu.getFirebase().auth());
  ui.start(id, config);
}
